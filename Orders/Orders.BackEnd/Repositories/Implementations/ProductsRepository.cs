...
namespace Orders.BackEnd.Repositories.Implementations
{
    public class ProductsRepository : GenericRepository<Product>, IProductsRepository
    {
        ...
        public override async Task<ActionResponse<int>> GetTotalPagesAsync(PaginationDTO pagination)
        {
            ...
            double conunt = await queryable.CountAsync();
            int totalPages = (int)Math.Ceiling(conunt / pagination.RecordsNumber);
        ...
        public async Task<ActionResponse<Product>> AddFullAsync(ProductDTO productDTO)
        {
            try
            {
                ...
                foreach (var productCategoryId in productDTO.ProductCategotyIds!)
                {
