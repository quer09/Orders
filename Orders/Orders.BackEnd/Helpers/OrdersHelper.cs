...
namespace Orders.BackEnd.Helpers
{
    public class OrdersHelper : IOrdersHelper
    {
        ...
        public async Task<ActionResponse<bool>> ProcessOrderAsync(string email, string remarks)
        {
            ...
            var responseTemporalOrders = await _temporalOrdersUnitOfWork.GetAsync(email);
            if (!responseTemporalOrders.WasSuccess)
            {
                return new ActionResponse<bool>
                {
                    WasSuccess = false,
            ...
            var temporalOrders = responseTemporalOrders.Result as List<TemporalOrder>;
            ...
            foreach (var temporalOrder in temporalOrders!)
            {
                ...
                var resposeProduct = await _productsUnitOfWork.GetAsync(temporalOrder.Product!.Id);
                if (resposeProduct.WasSuccess)
                {
                    var product = resposeProduct.Result;
                    if (product != null)
                    {
        ...
        private async Task<ActionResponse<bool>> CheckInventoryAsync(List<TemporalOrder> temporalOrders)
        {
            ...
            foreach (var temporalOrder in temporalOrders)
            {
                var responseProduct = await _productsUnitOfWork.GetAsync(temporalOrder.Product!.Id);
                if (!responseProduct.WasSuccess)
                {
